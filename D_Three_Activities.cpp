...
for(int i=0;i<=min(9LL,n-1);i++){
    for(int j=0;j<=min(9LL,n-1);j++){
        for(int l=0;l<=min(9LL,n-1);l++){
            if(a[i].se!=b[j].se && a[i].se!=c[l].se && b[j].se!=c[l].se){
                ans=max(ans,a[i].fi+b[j].fi+c[l].fi);
            }
        }
    }
}
