...
int n;
    ...
    for(int i=1;i<=n;i++){
dp[i]=dp[i-1];  // not deleting the curr element so ans is same as previous 
if(mp.find(v[i])!=mp.end())dp[i]=max(dp[i],1+i+mp[v[i]]); 
// deleting the element with another pair j  value same as v[i] (v[i]==v[j]) so dp[i]=dp[j-1]+i-j+1; 
// in map ve store (dp[j-1]-j) to reduce n2 complexity
if(mp.find(v[i])!=mp.end())mp[v[i]]=max(mp[v[i]],dp[i-1]-i); // updating map
else mp[v[i]]=dp[i-1]-i;
