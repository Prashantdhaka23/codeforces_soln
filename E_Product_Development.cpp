...
int dfs(int ind ,vector<vector<int>> & v ,vector<int>  vec){

     if(ind==v.size()){
        for(int i=0;i<vec.size();i++){
            if(vec[i]>0)return 1e14;
        }
        return 0;
     }  
     if(mp.find({ind,vec})!=mp.end())return mp[{ind,vec}];

    int ans= dfs(ind+1,v,vec);
    ...
    ans=min(ans,v[ind][0]+dfs(ind+1,v,vec));

    return mp[{ind,v2}]=ans;
