...
int g=abs(v[a-1].first-v[b-1].first)+abs(v[a-1].second-v[b-1].second);

int res2=1e17;
int res1=1e17;
for(int i=0;i<k;i++){
    int l=abs(v[a-1].first-v[i].first)+abs(v[a-1].second-v[i].second);
    int r=abs(v[b-1].first-v[i].first)+abs(v[b-1].second-v[i].second);
    res1=min(res1,l);
    res2=min(res2,r);

}

cout<<min(res1+res2,g)<<endl;
