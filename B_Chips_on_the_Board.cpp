...
int n,m; cin>>n;m=n; vector<int> v1(n);  vector<int> v2(m); for(int i=0;i<n;i++){  cin>>v1[i];
x+=v1[i];} for(int i=0;i<m;i++){  cin>>v2[i];
y+=v2[i];}

int a=*min_element(all(v1));
int b=*min_element(all(v2));

cout<<min(n*a+y,n*b+x)<<endl;
