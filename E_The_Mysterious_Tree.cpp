...
vi dfs(int node ,int par ,vector<vi> & adj ,vector<int> & v){
...
if(it!=par){
    vector<int> t= dfs(it,node,adj,v);
    
    for(int i=0;i<32;i++){
        if(t[i]>0 && temp[i]>0){
            res=max(res,t[i]+temp[i]-1);
        }
      if(t[i]>0)  temp[i]=max(t[i]+1,temp[i]);
    }
}
}
if(temp[x]>0){
     res=max(res,temp[x]-1LL);
}
temp[x]=max(temp[x],1LL);
...
for(int i=0;i<n;i++){       
    cin>>v[i+1];
    v[i+1]=__builtin_popcount(v[i+1]); 
      }
      vector<vi> adj(n+2);
