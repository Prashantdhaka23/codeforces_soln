...
int z2=-1;
int dp[200005];
int dfs(int ind,vector<int> & v, int prev){

if(ind==v.size())return 0;
if(dp[ind]==-1)return dp[ind];
int x=v[ind];
int ans=0;
if(prev==-1 || x==v[prev]+1){

ans=max(ans,1+dfs(ind+1,v,ind));

}else{

ans=max(ans,dfs(ind+1,v,prev));
