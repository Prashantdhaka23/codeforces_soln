...
int dfs(int node ,int par ,vector<vi> & adj,unordered_map<int,int> & mp,unordered_map<int,int> & mp2 ,unordered_map<int,int> & mp3 ,int a){
  mp2[node]=a;
  ...
  for(auto it : adj[node]){
    if(it!=par){
  ans=min(ans,1+dfs(it,node,adj,mp,mp2,mp3,a+1));
  b+=mp3[it];
    }
  }
