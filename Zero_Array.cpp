...
if(a==n)a=0;
// cout<<a<<" "<<b<<" "<<j<<endl;
auto it =mp.find({v[j],j});
auto it1 =mp.find({v[b],b});
auto it2 =mp.find({v[a],a});
...
if(v[a]>0)mp.insert({v[a],a});
if(v[b]>0)mp.insert({v[b],b});
if(v[j]>0)mp.insert({v[j],j});
