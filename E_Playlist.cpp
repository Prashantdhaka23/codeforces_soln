...
pii dfs(double  sum ,double k ,vector<int> & v){
...
// cout<<it.first<<" "<<it.second<<endl;
      ...
      b1+=it.first*(a1/it.second);

    }

    int d1=gcd(b1,a1);

    return mp[sum]={b1/d1,a1/d1};
}
...
int n; cin>>n>>k; vector<int> v(n);  for(int i=0;i<n;i++){       cin>>v[i];    }
...
//     cout<<res<<endl;
// }
  // Replace with your probability value
// ans=(p.fi*(inv(p.se)))%mod;
long long z=((p.fi*inv(p.se))%mod+mod)%mod;
