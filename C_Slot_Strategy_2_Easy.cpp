...
// vector<vector<int> > v1(10,vector<int>(3,n+1));
// vector<vector<int> > v2(10,vector<int>(3,n+1));
// vector<vector<int> > v3(10,vector<int>(3,n+1));
ans=1e5;
...
int ok=0;
map<int,int> a11,b11,c11;
for(int i=0;i<n;i++){
    mp1[a[i]-'0']=1;
    a11[a[i]-'0']++;
}
for(int i=0;i<n;i++){
  if(mp1.find(b[i]-'0')!=mp1.end())  mp1[b[i]-'0']=2;
    b11[b[i]-'0']++;
...
for(int i=0;i<n;i++){
  ...
  if(mp1.find(c[i]-'0')!=mp1.end() && mp1[c[i]-'0']>=2){
   
   int cnt=0;
   if(a11[a[i]-'0']>=2  || b11[b[i]-'0']>=2 || c11[c[i]-'0']>=2 ){
    ok=2;
    break;
   }
   ok=3;
