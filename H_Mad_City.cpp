...
//****** vector input end ******//
int p=0;
int dfs(int node ,int par ,vector<vi> & adj ,vector<int> & vis){
    vis[node]=1;

    for(auto it : adj[node]){
     if(it!=par){
       if(vis[it]==0) dfs(it,node,adj,vis);
       else p=it;
     }
    }
    return 0;
}
int dfs2(int node ,int par ,vector<vi> & adj ,vector<int> & vis,int x){
    vis[node]=1;
int d=0;
    for(auto it : adj[node]){
     if(it!=par){
       if(vis[it]==0) dfs2(it,node,adj,vis,x);
       else d=1;
     }
    }
    if(d==0){
...
vector<int> dis1(n+1,1e7);
vector<int> dis2(n+1,1e7);
queue<int> pq;
pq.push(a);
dis1[a]=0;
while(pq.size()>0){
