...
int ans=0;
    ...
    // Read and process the first set of values
    for (int i = 0; i < n; i++) {
        int x, y;
        cin >> x >> y;
        v.push_back({x, y});
    ...
    // Find the maximum change in values within the range [1, m]
    for (auto it = mp.begin(); it != mp.end(); ++it) {
        d += it->second;
        if (d > maxi && it->first >= 1 && it->first <= m) {
            maxi = d;
            val = it->first;
        }
         if (d < f2 && it->first >= 1 && it->first <= m) {
            f2 = d;
            // val = it->first;
        }
    }
    ...
    // Read and process the second set of values
    ...
    // Find the maximum and minimum change in values within the range [0, m]
    for (auto it = mp2.begin(); it != mp2.end(); ++it) {
        d += it->second;
        if (it->first >= 1 && it->first <= m) {
            if (d > maxi) {
                maxi = d;
            }
            if (d < mini) {
