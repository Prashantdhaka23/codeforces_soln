...
void dfs(int node ,vector<int> & vis,int p,vector<vector<int>> & adj,vi & v ,int & ans ,int res){
...
for(auto it : adj[node]){
    ...
    if(vis[it]==0 && v[it-1]>p){
        vis[it]=1;
        dfs(it,vis,v[it-1],adj,v,ans,res+1);
        vis[it]=0;
    }
}
}
