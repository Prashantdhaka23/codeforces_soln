...
//****** vector input end ******//
int dp[101][101][102];
int dfs(int ind, int k ,int prev, vi & v ,vector<vi> & paint){
   if(k<0)return 1e15;
   if(ind>=v.size()){
  if(k==0) return 0;
  else return 1e15;
   ...
   int ans=1e15;
   if(dp[ind][k][prev+1]!=-1)return dp[ind][k][prev+1];

   if(v[ind]!=0){
   if(v[ind]!=prev) ans=min(ans,dfs(ind+1,k-1,v[ind],v,paint));
   else  ans=min(ans,dfs(ind+1,k,v[ind],v,paint));

   }else{

    for(int i=0;i<paint[ind].size();i++){
        if((i+1)!=prev){
          ans=min(ans, paint[ind][i] +dfs(ind+1,k-1,i+1,v,paint));
        }else{

           ans=min(ans,  paint[ind][i] +dfs(ind+1,k,i+1,v,paint));
        }
    }

   }

   return dp[ind][k][prev+1]= ans;
...
vector<vi> paint(n,vector<int> (m,0));
