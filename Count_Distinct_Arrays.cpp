...
vector<int> findPreviousGreaterIndices(const vector<int>& nums) {
    int n = nums.size();
    vector<int> result(n, -1); // Initialize with -1 assuming no greater element found
...
vector<int> findNextGreaterIndices(const vector<int>& nums) {
    int n = nums.size();
    vector<int> result(n, -1); // Initialize with -1 assuming no greater element found
...
int n; cin>>n; vector<int> v(n);  for(int i=0;i<n;i++){       cin>>v[i];    }
 ...
 for(int i=0;i<n;i++)v1[i]=max(v1[i],v[i]);
...
if(i>0){
  ...
  if(v[i]<m)s.insert({v[i],i});
  m=max(m,v[i]);
 }

//  for(int i=1;i<n;i++)v2[i]=max(v2[i],v2[i-1]);
