...
mt19937 rng((unsigned int) chrono::steady_clock::now().time_since_epoch().count());
   ...
   segment tree se hoga 
   first sort all the values by {nums[i] -index,index};
    for each value find the max in the range from 0 to index-1  + nums[i]
    then update the value at that index
...
int n; cin>>n; n=2*n; vector<int> v(n);  for(int i=0;i<n;i++){       cin>>v[i];    }
