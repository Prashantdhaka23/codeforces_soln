...
// int dfs(int ind,vector<pair<int,int> > & v ,int a,int b,int m){
//     if(m==0 || ind==v.size())return 0;
//     if(dp[ind][m][a][b]!=-1)return dp[ind][m][a][b];

//  int ans=dfs(ind+1,v,a,b,m);
//  int a1=v[ind].fi,b1=v[ind].se;
...
//  ans=max(ans,d+dfs(ind+1,v,c,f,m-1));

//  return dp[ind][m][a][b]=ans;
...
int k=v[i];
 ...
 /*
dp[i][j][k] = maximum power of 2 that we can get if we take j from
the first i with power of 5 as k
*/
int dp[2][m+2][5222];
...
for(int i=0;i<n;i++){
   ...
   for(int j=0;j<=m;j++){
      ...
      for(int l=0;l<5200;l++){
            if(dp[i%2][j][l]==-1)continue;
        dp[(i+1)%2][j][l]=max(dp[(i+1)%2][j][l],dp[i%2][j][l]);
       if(l+v1[i].se>5200)continue;
       dp[(i+1)%2][j+1][l+v1[i].se]=max(dp[(i+1)%2][j+1][l+v1[i].se], dp[i%2][j][l]+v1[i].fi);
      }
