...
void find_stall(int t, vector<pair<int, vector<int>>> &cases) {
    for (auto &case_data : cases) {
        int n = case_data.first;
        vector<int> stall_data = case_data.second;
        int m = stall_data.size() / 2;
        int max_shakes = 0;
        int min_price = INT_MAX;
        int stall_index = -1;

        for (int i = 0; i < m; ++i) {
            int a_i = stall_data[2 * i];
            int q_i = stall_data[2 * i + 1];
            int shakes = n / q_i;
            if (shakes > a_i) {
                shakes = a_i;
            }
            if (shakes > max_shakes || (shakes == max_shakes && i < stall_index)) {
                max_shakes = shakes;
                min_price = q_i;
                stall_index = i + 1;
            }
...
int32_t main() {
    ...
    for (int i = 0; i < t; ++i) {
        int n, m;
        cin >> n >> m;
        vector<int> stall_data;
        for (int j = 0; j < m; ++j) {
            int a, q;
            cin >> a >> q;
            stall_data.push_back(a);
            stall_data.push_back(q);
        }
        cases.push_back({n, stall_data});
    }
