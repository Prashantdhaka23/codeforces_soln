...
int dfs2(int node ,int par ,vector<vi> & adj ,int c,vector<int> & dp,map<int,int> & mp,set<int> & st){
...
int dfs(int node ,int par ,vector<vi> & adj ,map<int,int> & mp,vector<int> & dp){
...
for(auto it : adj[node]){
    if(it!=par){
        ans=min(ans,1+dfs(it,node,adj,mp,dp));
    }
}
    // debug(ans);
