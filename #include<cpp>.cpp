...
int dfs(int ind, vector<int> &v, int ans,int p) {
    if (ind > v.size()) {
        return - p; // Return p instead of ans - p
    }
    ...
    for (int i = 0; i < v.size(); i++) {
        if (v[i] == 0) {
            v[i] = 1;
            int x = max(p, (i + 1) * ind); // Fix the x calculation
            res = max(res, (i + 1) * ind +dfs(ind + 1, v, ans-(i+1),x));
            v[i] = 0;
        }
    }
    return  res;
