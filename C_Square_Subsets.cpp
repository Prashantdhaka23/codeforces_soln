...
int dfs(int i,vector<int> & v ,int ans,vector<int > & vec){
    ...
    if(dp.find({i,ans})!=dp.end())return dp[{i,ans}];
   
    int res=0;
    int d=0;
    if(v[i]%2==0) d=pwr(2LL,v[i]-1);
    else{
        d=pwr(2LL,v[i]-1);
    }
    if(d==0)d=1;
    ...
    res+=(d*(dfs(i+1,v,ans,vec)))%mod;
    res=res%mod;
   if(v[i]>0) res+=(d*(dfs(i+1,v,ans^vec[i],vec)))%mod;
    res=res%mod;

     return dp[{i,ans}]=res%mod;
...
int n;
 cin>>n; vector<int> v(n);  for(int i=0;i<n;i++){    
       cin>>v[i];  
       mp[v[i]]++;  }
