...
string res="YES";
// User
int n,m;cin>>n; vector<vector<int>> arr(n+1,vector<int> (n+1,0));for(int i=1;i<=n;i++){ for(int j=1;j<=n;j++){  cin>>arr[i][j]; } }
...
for(int i=0;i<q;i++){
    ...
    cin>>a>>b>>c>>d;
    a++,b++,c++,d++;
    ans+=abs(arr[c][d]+arr[a-1][b-1]-arr[c][b-1]-arr[a-1][d]);
