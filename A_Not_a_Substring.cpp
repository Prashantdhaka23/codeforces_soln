...
//****** vector input end ******//
string rest="";
void dfs(string  t,string & s ,int sz ,int cnt){
    if(rest!="")return ;
 ...
 int x2=t.size();
  string d="";
 if(x2-x>=0)d=t.substr(x2-x,x);



 if(t.size()<s.size() || d!=s ){
 dfs(t,s,sz,cnt+1);
 }
...
if(rest.find(s)== string :: npos){
    cout<<"YES"<<endl;
    cout<<rest<<endl;
}else if(f.find(s)== string :: npos){
cout<<"YES"<<endl;
    cout<<f<<endl;
}else{
    cout<<"NO"<<endl;
