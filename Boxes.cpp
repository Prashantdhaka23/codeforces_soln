// you have  n dice  eack with k faces ?
// find out a way getting sum=s;
...
int dfs(int ind ,int sum ,int k ,int n){
...
// cout<<dfs(0,s,k,n)<<endl;
dp[0][0]=1;
 for(int i=1;i<=n;i++){
	for(int sum=1;sum<=s;sum++){
	for(int j=1;j<=k;j++){
  if(sum-j>=0)dp[i][sum]+=dp[i-1][sum-j];
