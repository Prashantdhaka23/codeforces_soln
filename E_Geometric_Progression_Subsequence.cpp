...
int solution(vector<int> &numbers) {
     // Initialize an array to store the maximum number for each digit mask
    vector<int> maxNumForMask(1024, -1);
   
    // Iterate through each number in the input vector
    for (int num : numbers) {
        // Calculate the digit mask for the current number
        int mask = 0;
        for (char c : to_string(num)) {
            mask |= 1 << (c - '0');
        }
        // Update the maximum number for the current digit mask
        maxNumForMask[mask] = max(maxNumForMask[mask], num);
    }

    // Initialize the maximum pair sum
    int maxPairSum = -1;

    // Iterate through all possible pairs of digit masks
    for (int i = 0; i < 1024; ++i) {
        if (maxNumForMask[i] <= 0) continue; // Skip if no number exists for this mask
        const int complementMask = 1023 ^ i;
        int tempMax = -1;
        // Iterate through all possible subsets of complementMask
        for (int j = complementMask; j; j = (j - 1) & complementMask) {
            tempMax = max(tempMax, maxNumForMask[j]);
        }
        // Update the maximum pair sum
        if (tempMax >= 0) {
            maxPairSum = max(maxPairSum, tempMax + maxNumForMask[i]);
        }
    }
...
for(int i=0;i<n;i++){
    cin>>v[i];
    mp2[v[i]]++;
    if(mp3.find(v[i])==mp3.end())mp3[v[i]]=i;
    if(mp2[v[i]]>=3){
        a=max(a,mp3[v[i]]+1);
    }
    
    for(int j=2;j<=30;j++){
        if(v[i]%j==0 && mp.find(v[i]/j)!=mp.end()){
            int x=v[i]/j;
            int y=mpp[v[i]/j].back();
            if(x%j==0 && mp.find(x/j)!=mp.end()){
