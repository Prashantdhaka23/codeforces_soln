...
//****** vector input end ******//
int f=0;
void dfs(int n ,int m ,int x,int y,map<int,string> & mp, vector<vector<int>> & vis,string & t,int d){
d=d%5;

if(t[d]!=mp[x][y]) return ;
...
void dfs2(int n ,int m ,int x,int y,map<int,string> & mp, vector<vector<int>> & vis,string & t,int d){
if(x==n-1 && y==m-1){
   f=1;
    return ;
}
...
d=d%5;
// if(t[d]!=mp[x][y]) return ;
if(x<n && y<m  && vis[x+1][y+1]==0 && x%5==d){
dfs2(n,m,x+1,y+1,mp,vis,t,d+1);
}
...
v[0][0]=1;
f=0;
dfs(n,m,0,0,mp,v,p,0);


// if(n==m)dfs2(n,m,0,0,mp,v2,p,0);
