...
for(int i=0;i<v1.size();i++){
     ...
     for(int j=0;j<v1[i];j++){
        unordered_map<int,int> mp;
         vi c;
       mp[v[k]]++;
            if(mp[v[k]]==1) c.pb(v[k]);
        }
        sort(all(c));
         if(c.size()>=2){
                int p=-1;
              for(auto it:c){
              if(p!=-1)  d=__gcd(d,it-p);
              p=it;
              }  
         }
         if(d==1)break;
        //  if(mp.size()==2 ){
        //     if(mp.rbegin()->first-mp.begin()->first<=1) ok=1;
        //  } 
