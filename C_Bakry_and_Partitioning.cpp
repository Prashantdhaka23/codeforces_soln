...
//****** vector input end ******//
int dfs2(int node ,int par ,vector<vi> & adj ,vector<int> & v,vector<int> & dp,int tot){
...
int dfs(int node ,int par ,vector<vi> & adj ,vector<int> & v,vector<int> & dp){


int xo=v[node];
    for(auto it : adj[node]){
        if(it!=par){
           int p=dfs(it,node,adj,v,dp);
           xo^=p;
        }
    }
