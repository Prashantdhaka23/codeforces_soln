...
void create(int ind ,int low ,int high,vector<int> & v){
...
int mid=(low+high)/2;
int ans=0;
create(2*ind+1,low,mid,v);
create(2*ind+2,mid+1,high,v);
seq[ind]=seq[2*ind+1]+seq[2*ind+2];
return ;

}
int sumq(int ind ,int low ,int high,int l ,int r){
if(high<l ||  low>r ){
return 0;
}
...
int mid=(low+high)/2;
int ans=0;
ans+=sumq(2*ind+1,low,mid,l,r);
ans+=sumq(2*ind+2,mid+1,high,l,r);

return ans;

}
void updateq(int ind ,int low ,int high,int p ,int x){
...
int mid=(low+high)/2;
int ans=0;
updateq(2*ind+1,low,mid,p,x);
updateq(2*ind+2,mid+1,high,p,x);
seq[ind]=seq[2*ind+1]+seq[2*ind+2];
...
int ind=0;
void dfs(int node,int par,vector<vector<int>> & adj,vector<int> & v){

mp[node].first=ind;
flat.pb(v[node]);
ind++;
for(auto it : adj[node]){
if(it!=par)dfs(it,node,adj,v);
}
mp[node].second=ind;
flat.pb(-1*v[node]);
ind++;
...
flat[r]=-x;
  ...
  updateq(0,0,flat.size()-1,l,x);
  updateq(0,0,flat.size()-1,r,-x);
...
int gg=sumq(0,0,flat.size()-1,0,l);
cout<<gg<<endl;
